...
const manuelBloch = (nodes, edges, sourceId, targetId) => {
  ...
  // Calculer la capacité minimale d'un chemin
  ...
  while ((path = findAugmentingPath()) !== null) {
    ...
    // Stocker les informations du chemin
    allPaths.push({
      path: path,
    ...
    for (let i = 0; i < path.length - 1; i++) {
      ...
      // Diminuer la capacité dans le sens direct
      residualGraph[u][v] -= pathCapacity;
      // Augmenter la capacité dans le sens inverse
  ...
  // Calculer le flot sur chaque arête originale
  ...
  // Trouver le meilleur chemin (celui avec la plus grande capacité)
  const bestPath = allPaths.length > 0 ?
    [...allPaths].sort((a, b) => b.capacity - a.capacity)[0] : null;
...
const showNotification = (message, type = 'info') => {
  ...
  notification.className = `fixed top-4 right-4 px-4 py-2 rounded shadow-lg z-50 ${type === 'success' ? 'bg-green-500 text-white' :
...
export default function App() {
  ...
  const calculateMaxFlow = useCallback(() => {
    ...
    // Exécuter l'algorithme Manuel Bloch
    const manuelResult = manuelBloch(nodes, edges, sourceNode.id, targetNode.id);

    if (manuelResult.maxFlow === 0) {
      showNotification('Aucun chemin de la source à la destination', 'error');
      return;
    }

    // Stocker les résultats de Manuel Bloch
    setManuelBlochResult(manuelResult);

    // Afficher les résultats de Manuel Bloch
    const pathsInfo = manuelResult.paths.slice(0, 5).map((pathInfo, index) =>
      `Chemin ${index + 1}: ${pathInfo.path.map(nodeId => nodes.find(n => n.id === nodeId)?.label).join(' → ')} (Capacité: ${pathInfo.capacity})`
    ).join('\n');

    showNotification(`Manuel Bloch - Flot maximal: ${manuelResult.maxFlow}\nChemins utilisés:\n${pathsInfo}`, 'info');

    // Exécuter Ford-Fulkerson pour comparaison
    setTimeout(() => {
      const fordResult = fordFulkerson(nodes, edges, sourceNode.id, targetNode.id);
      setMaxFlowValue(fordResult.maxFlow);
      setEdges(fordResult.flowEdges);
      setShowFlowResult(true);

      if (fordResult.maxFlow === manuelResult.maxFlow) {
        showNotification(`Ford-Fulkerson confirmé - Flot maximal: ${fordResult.maxFlow} ✓`, 'success');
      } else {
        showNotification(`Ford-Fulkerson - Flot maximal: ${fordResult.maxFlow} (Différence détectée!)`, 'error');
      }
    }, 3000);
  }, [nodes, edges]);
    ...
    return (
      ...
      >
        ...
        {edges.map((edge, i) => {
          ...
          if (showFlowResult) {
            if (edge.flow === 0) {
              strokeColor = '#ef4444'; // rouge - bloqué
            } else if (edge.flow === edge.capacity) {
              strokeColor = '#f59e0b'; // orange - saturé
            } else if (edge.flow > 0) {
              strokeColor = '#10b981'; // vert - avec flot
            }
          }
          ...
          return (
            <g key={i}>
              ...
              >
                {showFlowResult ? `${edge.flow}/${edge.capacity}` : `${edge.capacity}`}
              </text>
            </g>
          );
        })}
              ...
              <Card>
                ...
                <CardContent className="space-y-2">
                  <div className="text-sm text-gray-600">
                    <p><strong>Chemins trouvés:</strong> {manuelBlochResult.paths.length}</p>
                    {manuelBlochResult.recommendedPath && (
                      <div className="mt-2 p-2 bg-blue-50 rounded">
                        <p className="font-medium">Chemin recommandé:</p>
                        <p>{manuelBlochResult.recommendedPath.path.map(nodeId =>
                          nodes.find(n => n.id === nodeId)?.label
                        ).join(' → ')}</p>
                        <p>Capacité: {manuelBlochResult.recommendedPath.capacity}</p>
                      </div>
                    )}
                  </div>
                </CardContent>
